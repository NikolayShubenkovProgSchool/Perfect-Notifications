...
public class NotificationPusher {
    ...
    public static func addConfigurationAPNS(name: String, production: Bool, configurator: @escaping netConfigurator = { _ in }) {
        self.configurationsLock.doWithLock {
            self.iosConfigurations["\(name).isProd:\(production)"] = NotificationConfiguration(configurator: configurator, production: production)
        }
    }
    ...
    static func getStreamAPNS(configurationName name: String, production:Bool, callback: @escaping (HTTP2Client?, NotificationConfiguration?) -> ()) {
        var conf: NotificationConfiguration?
        self.configurationsLock.doWithLock {
            conf = self.iosConfigurations["\(name).isProd:\(production)"]
        }
    ...
    static func releaseStreamAPNS(configurationName name: String, production:Bool, net: HTTP2Client) {
        var conf: NotificationConfiguration?
        self.configurationsLock.doWithLock {
            conf = self.iosConfigurations["\(name).isProd:\(production)"]
        }
...
public extension NotificationPusher {
    ...
    public static func addConfigurationAPNS(name: String, production: Bool, keyId: String, teamId: String, privateKeyPath: String) {
        ...
        self.configurationsLock.doWithLock {
            self.iosConfigurations["\(name).isProd:\(production)"] = NotificationConfiguration(keyId: keyId, teamId: teamId, privateKeyPath: privateKeyPath, production: production)
        }
    }
}
...
public extension NotificationPusher {
    ...
    public func pushAPNS(configurationName: String, production:Bool, deviceToken: String, notificationItems: [APNSNotificationItem], callback: @escaping (NotificationResponse) -> ()) {
        pushAPNS(configurationName: configurationName, production:production, deviceTokens: [deviceToken], notificationItems: notificationItems, callback: { lst in callback(lst.first!) })
    }
    ...
    public func pushAPNS(configurationName: String, production:Bool, deviceTokens: [String],
    ...
    private func pushAPNS(configurationName: String, production:Bool, deviceTokens: [String], notificationItems: [APNSNotificationItem], attempts:Int, callback: @escaping ([NotificationResponse]) -> ()) {
        ...
        NotificationPusher.getStreamAPNS(configurationName: configurationName, production: production) {
            ...
            self.pushAPNS(c, config: config, deviceTokens: deviceTokens, notificationItems: notificationItems) {
                ...
                NotificationPusher.releaseStreamAPNS(configurationName: configurationName, production: production, net: c)
...
public extension NotificationPusher {
    ...
    public func pushIOS(configurationName: String, production:Bool, deviceToken: String, expiration: UInt32, priority: UInt8, notificationItems: [APNSNotificationItem], callback: @escaping (NotificationResponse) -> ()) {
        pushIOS(configurationName: configurationName, production:production, deviceTokens: [deviceToken], expiration: expiration, priority: priority, notificationItems: notificationItems, callback: { lst in callback(lst.first!) })
    }
    public func pushIOS(configurationName: String, production:Bool, deviceTokens: [String], expiration: UInt32, priority: UInt8, notificationItems: [APNSNotificationItem], callback: @escaping ([NotificationResponse]) -> ()) {
        ...
        self.pushAPNS(configurationName: configurationName, production:production, deviceTokens: deviceTokens, notificationItems: notificationItems, callback: callback)
    }
    //
}
